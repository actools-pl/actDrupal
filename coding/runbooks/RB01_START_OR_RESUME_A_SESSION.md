...
2. Select the recorded operating route and verify actual capabilities. For human-Git, inspect branch, commit and working-tree status using [RB02, step 1](RB02_RECEIVE_AND_APPLY_A_PATCH.md). For direct GitHub, read exact refs/tree using [RB12](RB12_DIRECT_GITHUB_OPERATIONS.md); do not assume prior-session write access persists. Compare with recorded state and [change control](../03_GITHUB_AND_CHANGE_CONTROL.md).
   - **Before first-start writes:** follow [RB11](RB11_NEW_REPOSITORY_STARTUP.md). Verify the destination identity, actual refs, permissions and applicable rules/automation; record the bounded BOOT-000 authority and reviewed seed contents before the affected write. Resolve a concrete unsafe trigger before that write. Any needed settings change requires its own applicable authority.
   - **As BOOT-000 proceeds:** record actual root, publication and integration-control results as they occur. Complete the verified BOOT-000 receipt before releasing BOOT-001; do not demand results before their producing steps.
   - **Once new-project source exists:** arrange independent backup under owner custody as RB11 and change control specify. Record actual coverage and gaps; a plan is not a verified backup. No old-repository backup or blanket pre-first-mutation backup gate is added.
   - **On resume:** reconcile valid completed startup receipts with current state and recheck relevant mutable refs, protections and trigger behavior before relying on them. Do not repeat completed bootstrap operations merely to complete records.
...
6. Choose the coordinator, coder or reviewer prompt for the actual step. Reviewer-only interface selection follows [CPD-12](../records/REVIEW_ROUTE_DECISION.md); it does not grant source-write authority. For a replacement conversation use [the restart prompt](../prompts/05_SESSION_RESTART.md).
