...
The source-check OS line is **Ubuntu 26.04 LTS x86_64** with CPython **3.14.7**, matching architecture D01. CP-001 local E01 qualification used clean Ubuntu Server **26.04.1 LTS** x86_64. Hosted Source CI remains the unchanged GitHub-hosted `ubuntu-26.04` x64 workflow. CP-003 does not broaden workflow triggers, permissions or runner authority.
...
`requirements/ci.in` pins eight direct inputs: the five CP-001 source/test tools plus the three CP-002 runtime dependencies:
...
CP-003 adds no dependency and does not regenerate either requirements file. The
accepted normalized generation command remains:
...
`tools/check_source.py` retains the CP-001 fail-closed workflow/scanner/CLI policy,
the frozen CP-002 dependency contract, and evolves package truth for CP-003:
...
4. the complete test suite, including the CP-003 catalog, graph, operation and
   diagnostic-evaluation checks, strict parser/default/origin/JCS adversarial
   tests and all unchanged CP-001 CLI tests;
5. the two CP-001 controlled negative build fixtures, still reason-bound;
6. one closed **30-member** wheel: `actools/__init__.py`, `actools/cli.py`, eight
   `actools.contracts` Python modules, eleven schema/default JSON resources, two
   policy JSON resources, and the same seven distribution metadata/license
   members;
7. exact wheel metadata runtime requirements: `PyYAML==6.0.3`, `jsonschema==4.26.0`, `rfc8785==0.1.4`;
8. byte equality for every packaged contract module, schema/default/policy
   resource, and root license/notice;
9. an actual installed `actools` launcher in a fresh isolated virtual environment
   outside the source tree: that verifier environment first hash-installs the
   complete actual `requirements/ci.lock` and passes `pip check`, then installs
   only the already-built Actools wheel with `--no-index --no-deps`; the installed
   probe preserves CP-001 version/help/error behavior, proves Draft 2020-12
   configuration resolution, loads the nine-family catalog and 163-node graph
   from packaged resources, checks the exact three runtime versions, and requires
   every runtime distribution/module path to resolve inside that fresh verifier
   environment rather than from an outer/system site-packages location;
...
## Targeted CP-003 and compatibility commands

With the accepted lock installed in the exact environment, run the frozen CP-003
targeted command:
...
Then run the frozen compatibility command:
...
Source-CI success does not establish Drupal installation, privileged execution,
host hardening, firewall/SSH behavior, diagnostic collection/HTML rendering,
backup restoration, release signing or production admission. CP-003 is a pure
graph/contract/evaluator slice and does not mark G03/G04/G09/G19 or any
product-wide gate PASS by itself.
