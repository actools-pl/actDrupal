...
def test_lock_header_policy_accepts_only_truthful_normalized_command() -> None:
    exact = (
        "#\n# This file is autogenerated by pip-compile with Python 3.14\n"
        "# by the following command:\n#\n"
        f"#    {EXPECTED_LOCK_COMPILE_COMMAND}\n#\n"
        "--only-binary :all:\n"
    )
    verify_lock_header(exact)
    ...
    with pytest.raises(CheckFailure):
        verify_lock_header(false)


def test_current_lock_header_and_body_match_frozen_contract() -> None:
    text = (ROOT / "requirements" / "ci.lock").read_text(encoding="utf-8")
    verify_lock_header(text)
    assert lock_body_sha256(text) == EXPECTED_LOCK_BODY_SHA256


def test_current_lock_dependency_hash_body_is_still_frozen_before_regeneration() -> None:
    text = (ROOT / "requirements" / "ci.lock").read_text(encoding="utf-8")
    assert lock_body_sha256(text) == (
