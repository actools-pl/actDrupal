...
A fresh role-appropriate conversation receives or reads the relevant current files and produces one bounded result. Coordination/coding remain ordinary-chat; independent/documentation review may use the approved Work route or ordinary fallback in [CPD-12](records/REVIEW_ROUTE_DECISION.md). Choose and record one route for the task: **human-Git delivery**, where the operator saves and applies a patch or complete files, or **direct GitHub delivery**, where a currently available integration performs already authorized repository operations on the named task branch. The human retains integration authority and operates the test machines. The coordination/coding preference remains **5.6 sol / Extra High**, if available. Reviewer selection follows the operator-reported **Work / ChatGPT 6 Astra** preference and ordinary **5.6 sol** fallback; record actual observed or reported settings and unknown effort explicitly. No Work window, ChatGPT Project, shared project memory, connector, background job, automatic repository inspection or SSH capability is required.
...
## Reviewer usage fallback

A usage-triggered reviewer switch between the two CPD-12 choices does not need renewed approval. Before switching, retain the exact candidate/tree, complete source/diff, packet revision, open findings, partial reading boundaries and actual check receipts outside the candidate. The replacement reviewer uses a fresh conversation, verifies its own access, attributes previous work and records what it read or tested itself. It must not invent completion of missing reading or turn an unfinished review into acceptance. A switch does not authorize any repository or server effect. Use [CPD-12](records/REVIEW_ROUTE_DECISION.md) and the [restart prompt](prompts/05_SESSION_RESTART.md).
