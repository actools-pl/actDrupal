...
The coordinator's isolated environment is Python 3.13.5 and currently has no package-index network path. The operator prepared a disposable Ubuntu Server 26.04.1 LTS x86_64 host with uv-managed CPython 3.14.7 for the authorized E01 run. E01 Phase 1C has now generated the resolver-backed lock from exact `requirements/ci.in` using pip 26.2.1 + pip-tools 7.6.1 with pip configuration disabled, explicit PyPI resolution, `--allow-unsafe`, hashes, backtracking and wheel-only policy. Hash-locked install/build/audit remain pending. A required scanner error/unavailable result cannot be converted into PASS.
...
- Exact CPython 3.14.7 lock regeneration/compare is complete on the disposable Ubuntu 26.04.1 E01 host. Phase 1C input `ci.in` SHA-256 is `615a1f8c33300e7b47c366916c89738bd572b37b8124e98064efe1fca27f86f8`; resolver-generated lock SHA-256 is `6836c3ed72a3667e97b6901f9836f6c0957dd3512911b221986bc7c1b223e7e7` with 34 distributions. Relative to the handwritten lock it removes `typing-extensions`, canonically emits `boolean-py`, and updates `cachecontrol`, `msgpack` and `pip-api`. Hash-locked install, exact tool/version consistency, complete `python tools/check_source.py` and real pip-audit 2.10.1 output remain pending on the exact new candidate containing this lock. GitHub-hosted `ubuntu-26.04` source CI remains later and requires separate publication/PR authority; the hosted runner label is public preview at this correction point.
