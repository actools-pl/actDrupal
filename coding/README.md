# Actools Coding Package v1.3

Start with [00_START_HERE_CODER_DOCUMENTATION.md](00_START_HERE_CODER_DOCUMENTATION.md).
...
This download contains process documents and reviewed read-only process helpers. No installer, GitHub bootstrap, server setup or runtime qualification has been executed by creating it. The original v1.5 under `reference/` is historical evidence only. CP-038/039 are cancelled identifiers, and active dependencies exclude them. Backup/restore, disaster recovery and supported updates for newly created sites remain product requirements.
