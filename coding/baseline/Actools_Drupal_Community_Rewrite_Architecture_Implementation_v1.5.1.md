...
Coding uses ordinary ChatGPT outside Work and Projects under Coding Package v1.3. It does **not** require an OpenAI service or API key in the deployed community edition. Runtime AI and content-intelligence services are deferred from the first release under C01/F69–F70; coding through Codex does not enable them.
...
| Recovery and lifecycle | Independently committed complete recovery sets; transaction and file coverage; safe copies, revocation, deployment and recovery promotion | §§7.11, 10.5–10.7, 11.1–11.3, 16.1 |
...
Existing accepted platform/access decisions and S2–S5 requirements are preserved. Generic document safety requirements remain conditional on enabling a document capability; they do not add domain functionality to this project. The first-release scope and policies are collected in §§18.2–18.8, as revised for fresh installations in §§1.5 and 18.11; accepted recovery and access decisions remain recorded in §18.1. The later user-authorized operator UX addition is integrated in §18.10 and Annex UX. It improves the installer/operator experience without adding domain application screens or a first-release browser service.
...
### 1.5 V1.5.1 — New repository and fresh installations only

**Authority:** the user selected a new independent repository and then explicitly selected fresh installations only, with no migrations, on 8 September 2026. This is an accepted scope change. V1.5.1 supersedes v1.5 for implementation; the original is historical provenance only.

- Canonical development repository: https://github.com/actools-pl/actDrupal. Start with new Git history, use `main` for reviewed integration and short-lived `task/<ID>` branches. Record the actual initial commit at execution; no old commit, preservation tag or ancestry is a prerequisite.
- Install a new Drupal website on a qualified clean host. Existing installations, foreign database/files archives and old installer configuration are not discovery/import sources. Refuse conflicting populated targets before mutation; do not erase or adopt them automatically.
- Exclude existing-site migration, legacy discovery/mapping, site import, migration cutover and legacy `age` backup readers. These are excluded from the current product scope, not postponed tasks that block the release. Any future addition needs a separate scope decision.
- Retain backup and restore of installations created by this installer, including disaster recovery onto a replacement qualified clean host. Retain supported updates, necessary internal database/contract schema changes, and trusted backup repository re-encryption after key compromise. These are lifecycle and recovery operations for this product, not existing-site import.
- Keep the `actools` CLI name, selected Python/Ansible/Compose/PHP design, Ubuntu target, security/UX controls, accepted capacity qualification profile and recovery objectives. A fresh site may begin empty; the representative 80 GB recovery/load qualification target is not an instruction to import a live site or prepopulate production.

The coding package implements this decision in RB11, BOOT-001, WP01, WP24, task dependencies and release applicability. CP-038/CP-039 are cancelled scope records; active tasks must not depend on them. Historic S4-RI20/S4-FI40 migration entries are excluded, never marked PASS. The scope qualification cases are in §18.11.

The archived v1.5 report may be consulted to trace a historical finding. Its old repository instructions, migration decisions, source code and historical check results are not current implementation instructions or evidence for actDrupal.

## 2. Evidence, provenance and limits

### 2.1 Current baseline and historical evidence

The active source repository is https://github.com/actools-pl/actDrupal. Its exact implementation commit is UNSET until repository startup is executed and recorded. The new project does not inherit another repository's code, refs, CI results or completed tasks.

This report derives from the user-supplied v1.5 architecture. Its original source inventory, pinned historical URLs and review observations are retained unchanged in `reference/Architecture_v1.5_ORIGINAL_REFERENCE_ONLY.md` inside Coding Package v1.3. That report has SHA-256 `3b5873db1c30687b63c51f1f8078d80f3ca2381698ec305444533222ba3a5100`. The package's `baseline/SOURCE_RECORD.md` records both input and revised identities.

Historical feature and finding tables below retain useful evidence about why controls were specified; they are not claims that the new repository already contains those files. No old source checkout or live-site inventory is required to begin the new implementation. Use synthetic fixtures for the applicable safety contracts; any deliberate future source reuse needs its own review and attribution.

### 2.2 What verification does and does not establish

The original report records syntax checks and historical workflow observations for the former implementation. Those results were not rerun in this revision and are not qualification evidence for the new repository. This revision changes documentation and task scope only; it does not implement or test the installer, Ubuntu support, recovery, capacity or security controls.
...
| F02 | Legacy `fresh`, per-environment install, installer update | Live; overlapping paths in installer and CLI | Core behavior through one command engine; only the canonical new command interface; no old-installer compatibility aliases |
...
| F36 | Historical age key generation and encrypted backup | Historical implementation used opt-in age archives | New product uses Restic under D08. Legacy age reader/import is excluded by v1.5.1; retain independent recovery custody for the new format. |
...
| F39 | Manual restore with encrypted-file handling | Live, including `.age`; unsafe destructive ordering remains | Core staged restore, validation before recovery promotion, writer quiesce, explicit target and safety snapshot |
...
| F44 | Automated update rollback | Planned; manual rollback exists | Rebuild only with schema compatibility classification; never blindly reverse a database schema update |
...
Source paths and line references in this historical findings table refer to the snapshot identified in the archived v1.5 report, not to files already present in actDrupal. The original Appendix A retains their pinned citations.
...
| R05 | P0, live update safety | `cli/actools:163–194`, `actools.sh:640–661`: snapshot failure tolerated; installer schema-update failure can be tolerated | One update engine; validated complete recovery point required; failed schema update stops rollout and returns failure |
...
| Database | Latest patched **MariaDB 11.4 LTS** as a conservative initial container candidate | Confirm release support, Drupal/module compatibility, TLS, backup/replay tools and schema-update tests; choose a newer LTS only through qualification |
...
| Recovery engine | **Accepted: Restic**, with encryption/repository client in a separate trusted backup environment; Actools owns MariaDB-aware capture; PITR orchestration is deferred under C01 | Pin/qualify Restic version and backend; accepted D08 key/deletion separation, full restore and rejection of unsupported foreign/legacy recovery inputs |
...
| `research/reference/` | Historical references clearly labeled non-runtime; excluded from deployed packages |
...
Use strict YAML/JSON with safe parsing, no custom executable YAML tags, no shell substitution and no runtime Python/PHP import paths from config. Unknown keys and unsupported combinations fail validation. Secrets are file/secret references, not inline values in routine config. Unsupported or removed flags must fail explicitly; they cannot silently select a default.

Every capability record includes dependencies, install stages, resources, required secrets, permitted endpoints, health probes, schema updates, backup constituents, rollback/disable behavior, support status and acceptance-test IDs. Schema validation must reject combinations the release does not support. For example: cache disabled removes cache service, dependencies and Drupal settings together; remote rendering requires verified TLS; object storage requires its recovery adapter.
...
Maintain a credential graph by environment, consumer, phase, generation, issuer and retirement owner. Build-fetch, schema-update, runtime, backup, diagnostics and administrative authority are distinct; build secrets cannot persist in final artifacts and schema-update credentials cannot leak into steady-state services. Runtime grants must support the actual qualified Drupal workload without global administration.
...
Application rollout: validate compatibility and target artifact signatures → check capacity and permissions → create/verify a complete recovery checkpoint → rehearse significant schema update on an isolated restored copy → drain jobs and restrict writers as required → deploy immutable target → apply schema updates → run readiness/security/data checks → resume traffic/jobs → retain previous release/checkpoint and report result.

Classify schema updates as backward-compatible, forward-only or requiring recovery. Automatic code rollback is allowed only when the schema/data contract supports it. A failed forward-only schema update leaves the site in a controlled recovery state with a clear operator action; it must not silently reintroduce old code against an incompatible schema or discard acknowledged writes.
...
## 13. How to code this project using the ordinary-chat coding package
...
| M6 — Safe lifecycle | Canonical updates, schema-update policy, rollback/recovery, release/self-update | Failed-build/snapshot/schema-update/readiness tests preserve recoverability |
...
| M9 — Community release | Fresh-install and recovery guides, contributor process, support policy, signed artifacts and runbooks | Release acceptance matrix complete; no supported feature left as a stub |
...
| WP21 | Canonical application/schema update and management upgrade | Failed checkpoint/schema-update/readiness cannot report success; tested compatible rollback |
...
| WP24 | Fresh-install lifecycle qualification and full disaster-recovery exercise | Reject conflicting/foreign installation inputs; prove own-site private restore, qualified clean-host recovery, writer control and recovery promotion. No existing-site importer. |
...
| O07 — Tenancy — deferred under C01 | Per-site isolation, quotas, lifecycle/recovery | M3/M4/M6; cross-tenant access and resource-exhaustion tests |
...
|---|---|
| WP01–WP02 | Requirement graph, decision/implementation separation, strict canonical parsers, contract readers/schema-updates and finite capability profiles. |
...
| WP24–WP25 | Fresh-install qualification and independent own-site DR; installed artifacts, release trust, finite support/retirement and rehearsed operator runbooks. |
...
| G18 — Update failure | Failed backup/build/schema-update/readiness; incompatible schema | No false success; safe state and tested recovery without silent loss of acknowledged data |
...
| RV13-T12 | Interrupt trusted-set re-encryption after synthetic key compromise | Coverage and retention stay honest; source history is not prematurely deleted; untrusted sets are not blessed | WP11/14/15/25; G08/14/15/16 |
...
## 16. Fresh-install boundary and recovery continuity

Existing-site migration is excluded by the user's v1.5.1 decision (§1.5), superseding C05/D13. Ship no legacy inventory/import tool, old shell-configuration parser, foreign-site mapper, legacy `age` reader, or migration cutover command. Do not make access to the historical repository or any existing site a prerequisite for coding or release acceptance.

The installer verifies a supported clean host, explicit installation/site identities and a reviewed deployment plan. Discovering conflicting existing services, application data or populated managed destinations must stop the affected installation before changes and report the conflict without printing secrets. It does not authorize deletion, conversion, adoption or host reimaging. A documented idempotent resume may operate on the same verified installation's own journal and resources; it must not adopt an unrelated installation by path alone.

### 16.1 Backup, restore and updates for newly created installations

Newly installed sites retain the complete qualified backup and recovery workflow in §10, including independent custody, private restore, writer suppression, reconciliation, revocation enforcement and promotion. Restoring a supported authenticated Actools recovery set onto a qualified replacement host is disaster recovery. A `.sql` file, file archive or Restic repository by itself is not proof of a supported Actools recovery set.

Validate producer/format versions, installation/site identity, trusted manifest, constituents and supported recovery edge before effects. Reject foreign or legacy material without executing source code or silently adding an importer. Test data and fixtures must be synthetic or otherwise explicitly authorized and safe.

Supported product/application updates and internal schema evolution retain §11's recovery checkpoints and failure behavior. Future production upgrade claims require an actual tested source-to-target release edge. During restore promotion and compatible rollback, preserve one authorized writer and acknowledged data under the existing recovery contracts. These duties are independent of the excluded existing-site migration workflow.
...
**Status: C01–C04/C06 are retained; C05/D13 is superseded by the fresh-install-only scope; v1.5.1 is the active architecture and implementation baseline with the separately user-authorized UX addition (§18.10).** Previously accepted D01–D11 portions remain accepted. Configurable HTTP/3 and the bounded F10 Caddy rate-limit integration are included subject to their qualification gates. No feature is newly removed. D05/D06 remain the recorded generic future-integration boundary and deferred worker placement.
...
| C05 — Fresh installations only / D13 | **Revised by user, 8 September 2026:** exclude existing-site discovery/mapping/import, migration cutover and legacy backup import. | Install new sites on qualified clean hosts. Retain own-site backup, restore, disaster recovery and supported updates; no migration task blocks the release. |
...
| F36 | Changed by user: Restic with keys off production; legacy age import excluded in v1.5.1 | D08 custody retained; C05/D13 fresh-install-only decision removes legacy readers | New encryption/repository work remains off production. Accept only qualified recovery sets produced by supported releases of this installer. WP14–WP15, WP24. |
...
| Installation boundary | Fresh installation rejects foreign populated targets; recovery accepts only qualified native Actools sets. No existing-site discovery/import/cutover command or action is exposed. |
...
Then WP10 pins and generates the runtime; WP11 establishes DB/cache/secret lifecycle; WP12 installs and protects Drupal; WP13 proves storage behavior. WP14–WP15 provide complete capture and isolated recovery before any destructive lifecycle acceptance. WP16 remains deferred from the first release; it joins only a later explicitly selected and qualified PITR scope. WP21 depends on proven checkpoints/recovery and compatible schemas; WP24 proves fresh-install conflicts and independent own-site recovery; it does not implement migration. WP22/23 collectors, truthful coverage, history and selected notifications grow with each slice; their final integration is later, never their first appearance. WP25 packaging starts with slice 1 and concludes with the release matrix and rehearsed runbooks. Conditional document work has no dependency on initial platform admission.
...
| Source layout | Independent new repository https://github.com/actools-pl/actDrupal; new root history, `main` integration, `task/<ID>` branches | WP01 records the actual bootstrap SHA and current instructions; no inherited base or source import. Startup runs only through the activated coding-package task. |
...
Support fresh installation for the first release and qualified recovery of sites created by this installer. Existing-site imports are excluded. After a second release exists, advertise only explicitly tested source-to-target upgrade edges, initially the immediately preceding supported Actools release to the current one. An upstream major-version jump, Ubuntu release upgrade or database/schema downgrade has no implicit support. Required intermediate upgrades and independently retained recovery readers are published with each release. Release support has an owner and dated dependency limits; a deadline never silently deletes data or stops a website.

For D13, qualify fresh installation only and enforce §16 at all command, parser, plan/action and recovery entrypoints. Historical site inventory, importer support and migration cutover are outside the product. Preserve own-site recovery and update guarantees independently, using controlled qualification fixtures.
...
| Restic capture byte proof, command/lock/cancellation matrix and key-incident repository re-encryption | WP14/WP15; before production capture, maintenance or compromise-recovery claims. |
...
| Fresh-install conflict fixtures and supported native recovery-set versions | WP09/WP15/WP24 before installation/recovery qualification | Use controlled fixtures; reject unrelated existing sites and unsupported/foreign backup material before effects. No live migration inventory is required. |
...
### 18.11 V1.5.1 change record and scope qualification

**Accepted 8 September 2026:** new independent actDrupal repository; fresh installations only; no existing-site migration or legacy backup import. §§1.5/16 supersede the earlier source-layout choice and C05/D13. F36 retains new Restic recovery while removing the old age reader. WP24 retains fresh-install and full DR qualification. S4-RI20/S4-FI40 are excluded historical migration entries. All other applicable safety controls and G gates remain binding.

| Case | Proposed acceptance assertion | Owner / applicable gates |
|---|---|---|
| FRESH-T01 | A qualified clean host can receive a new installation; conflicting existing services/data/managed paths cause an actionable failure before affected mutation. A resume binds the existing journal/resource identity rather than adopting foreign resources. | WP09/WP12/WP24; G01/G02/G03/G09 |
| FRESH-T02 | CLI help, action registry, configuration readers and release support advertise no existing-site discovery/import/cutover or legacy age reader. Removed entrypoints are rejected before effects. | WP02/WP03/WP09/WP24; G03/G09/G21 |
| FRESH-T03 | Foreign or legacy backup/SQL/files inputs cannot enter native restore merely by matching a filename or backend. Supported producer/format, authenticity, identity and complete-set validation are required before effects. | WP14/WP15/WP24; G03/G07/G08/G15 |
| FRESH-T04 | A site created by the new installer can be backed up and privately restored on a qualified replacement host with writer controls, revocation and acknowledged-data guarantees; complete RPO/RTO proof remains required. | WP14/WP15/WP23/WP24; G14/G15/G16/G18 |

These cases are planned assertions, not executed tests. CP-038/CP-039 are cancelled scope records and are removed from active dependencies. CP-040 follows CP-037; CP-050 follows CP-049 and CP-027, with proven recovery inherited through its existing chain. Excluded migration cases are NOT_APPLICABLE by scope, never passing evidence. Native backup re-encryption and internal schema updates retain their independent existing tests.

This is a documentation revision. No repository write, installer build, server action or runtime qualification occurred in preparing it.

## Appendix A. Historical source map

The unchanged v1.5 report in `reference/Architecture_v1.5_ORIGINAL_REFERENCE_ONLY.md` inside Coding Package v1.3 contains the pinned source map for the historical findings. This active document does not redirect those citations to unrelated paths in the new repository. Consulting that historical map is optional research; it does not require a legacy checkout, code import, migration tool or inherited Git ancestry.

Current implementation evidence must name actual actDrupal files and full commits after they exist. Its tests, manifests and support claims are produced anew.
...
**Status:** adopted architecture requirements and conditional capability requirements. These tables record design integration, not implementation, completed tests, reproduced exploits or production approval. The source’s repository findings concern the historical snapshot identified in the archived v1.5 report, not a current actDrupal commit. The `S2-` prefix identifies the source while preserving its original control IDs. Base F/L/R/WP/G identifiers retain their existing meaning; section targets refer to the corresponding architecture subjects.
...
| S2-AUD-DB-02 | Adopted design: app/schema-update/backup/metrics/root grants and rotation; app readiness under app identity. | §7.6,8,12; WP11,WP22; G07,G08,G10 |
...
| S3-TEST-X08 | Security/config update with in-flight old FPM request, CLI/cron/worker/cache generation and rollback prevents obsolete publication and preserves schema/current-policy compatibility. | G18/G22 |
...
| S4-RI02 | Baseline — adopt | Persisted contracts SHALL version readers/writers/schema-updates, reject duplicate keys, preserve omitted/null/false/explicit semantics and leave unsupported future documents unchanged. | §§7.3–7.5,11,16 | WP02, WP04, WP09, WP21, WP24 | G02, G03, G09, G18 |
...
| S4-RI20 | Excluded in v1.5.1 — historical existing-site migration requirement | Retained ID for traceability only; no discovery/import/cutover implementation or release prerequisite. Own-site restore/rollback writer protection remains governed by §§10–11 and 16.1. | §§1.5,16 | WP24 scope record | NOT_APPLICABLE to current migration-excluded profile; not PASS |
...
| S4-FI04 | S4-RI02 | Mix CLI/executor/schema versions and interrupt state schema update | Only documented combinations proceed; a compatible inspect/recovery path remains available | G02, G03, G09, G18 |
...
| S4-FI40 | S4-RI20 | Historical existing-site migration cutover test, excluded in v1.5.1 | NOT_APPLICABLE by accepted scope; no execution/PASS claim. Own-site recovery promotion and rollback still require their independent tests. | Excluded case; applicable G15/G16/G18 recovery/update gates remain |
...
|---|---|---|---|
| S4-A | Requirement graph and contract evolution; S4-RI01–S4-RI02 | WP01–WP02 | Source/decision/test graph, supported readers/writers and semantic schema-update fixtures defined. |
...
| S4-H | Maintenance and recovery release gate; S4-RI21; S4-RI20 excluded | WP24–WP25 | Fresh-install/own-site recovery walkthrough, update failure protection and support/retirement paths verified; no real-site migration input required. |

These groups extend existing WPs; they are not a second roadmap. Packaging, CI, baseline diagnostics and documentation start with the early slices. Linux preparation remains the first deployment deliverable. S4-C develops against isolated fixtures; production updates/schema changes remain blocked until complete verified recovery prerequisites in S4-D pass. Conditional PITR/document/provider tests do not enable optional scope.
...
| S5-BH06 | Integrated; core refinement | Declare each credential's environment, permitted consumers/phases, inheritance, rotation generation and retirement owner; production credentials are non-inheritable by default. Separate build/runtime/schema-update/backup/diagnostic/worker contexts. Generate service mounts/networks from the relationship graph, then verify effective access and actual grants separately from redaction. Preserve sufficient qualified Drupal runtime privileges. Build secrets cannot persist in final artifacts; secret mounts do not authorize hostile build execution. | H05/H11/I04/I05/AUD-DB-01/02; §§7.5–7.6, 11 | WP02/WP10/WP11/WP18 | G07/G08/G09/G22 | S5-BHT13–14 |
...
| S5-BHT13 | S5-BH06 | Distinct build/runtime/schema-update/backup/worker secret canaries; layers/logs/env/mounts: only approved consumers/phases obtain them; redaction and access isolation pass independently. | WP10/WP11/WP18 | G07/G08/G22 |
...
| S5-BHT28 | S5-BH12 | Active backup/restore/schema-update, stale reservation and crash after repair intent: interlocks and attempt accounting survive; reconciliation precedes retry/reset. | WP04/WP21/WP22/WP23 | G02/G14/G15/G18 |
...
Implement S5-BH01–04/06 in the foundational schemas/executor before additional handlers; S5-BH05/07 with recovery and supported updates; S5-BH08–12 alongside the first usable affected components. Existing host, HTTP, confinement and backup gates remain required from first deployment, without waiting for a later audit milestone.
...
The existing fifteen-minute validity window governs **starting** a plan. It is not a timer that aborts an already authorized restore or schema update. Long operations follow stage deadlines and checkpoints.
