...
- CP-003 review corrections now bind serialized gates to the pure evaluator, propagate prerequisite failures, require complete backup/journal proof relationships, enforce calendar-valid nanosecond UTC and the fifteen-minute plan window, select the Restic REST transport, validate exact guidance tuples and deferred gate scope, and define bounded resource loading plus a noncircular reviewed-plan digest.
- CP-003 diagnostic serialization now preserves raw skipped evidence while accepting only the shared evaluator's dependency-adjusted `UNKNOWN` outcome and rejecting forged status, observation, coverage and gate data.
