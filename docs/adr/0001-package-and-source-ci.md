...
- Source CI is the single exact workflow `.github/workflows/source-ci.yml`, limited to pull requests to `main` and pushes to `main`, `contents: read`, GitHub-hosted `ubuntu-24.04`, Python 3.14.7, immutable action pins and non-persisted checkout credentials. The checker accepts the complete approved shape, not substring presence.
